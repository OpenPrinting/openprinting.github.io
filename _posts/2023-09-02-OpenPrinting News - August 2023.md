...
What was missing is the desktop integration. I have tried to rush in the changes in the "Printers" module of GNOME Control Center getting it just working without any UI design fancinesses and put Mohit Verma under some pressure, but this turned out not being that easy.
...
This caused a lot of discussion in a [Ubuntu Discourse thread](https://discourse.ubuntu.com/t/cups-snap-call-for-testing/), especially the leaders of several Ubuntu flavors chimed in. I tried to calm the people down and bring in ideas for quick solutions, like providing a modified GNOME Control Center with all modules but the "Printers" module removed for example, which could be installed as independent printer setup tool on any distro.

But this does not cover everything and already being after Feature Freeze it would be impossible for all these flavors getting ready for the New Architecture, and therefore we have pulled back. Especially also to avoid a disaster as we had with Firefox.
