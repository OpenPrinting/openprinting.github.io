...
Microsoft did not present any concept to support non-drierless legacy printers, considering all these printers obsolete. But in reality, at least the ones for which there are drivers under Linux can be continued to be used, thanks to WSL and Printer Applications. Here is [how to do this](https://openprinting.github.io/wsl-printer-app/).
...
We had some interesting discussions in the last weeks. See the [mailing list archives](https://lists.linuxfoundation.org/pipermail/printing-architecture/2023/thread.html). The following links are to the initial posting of each thread:

- [PDFio: Replace QPDF by PDFio in libcupsfilters as GSoC project?](https://lists.linuxfoundation.org/pipermail/printing-architecture/2023/004369.html): Should we replace the C++ PDF manipulation library QPDF by the standard-C PDFio written by Mike Sweet?
- [RFC: Pantum M7300FDW and similar](https://lists.linuxfoundation.org/pipermail/printing-architecture/2023/004378.html): Alexander Pevzner has discovered a firmware bug in Pantum printers which breaks their driverless IPP printing support. Should one implement a workaround here?
- [github.com/OpenPrinting/goipp 1.1.0 and ipp-usb 0.9.24 announce](https://lists.linuxfoundation.org/pipermail/printing-architecture/2023/004375.html): Alexander Pevzner resumes development of our IPP-over-USB implementation ipp-usb and goipp
